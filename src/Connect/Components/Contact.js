...
const base = new Airtable({ apiKey: 'key80rQbZcgw1Kmez' }).base('appn1ZhFA6zPF57JI')
...
export default class Contact extends Component {
  ...
  submit = () => {
    const { name, email, role, r1, r2 } = this.state
  ...
  render() {
    const { name, email, role, r2, r1 } = this.state
    return (
      <Fade>
        <div className="card" id="contact-card">
          ...
          <p style={{ fontWeight: 'normal' }}>
            At The Bruin Review we're looking for passionate and innovate people to join our team. If that's you, fill
            out the application below. Learn more about The Bruin Review in our virtual info session to the left.
            Applications due by midnight Sunday, April 12.
          </p>
          {this.state.showForm ? (
            <Fade when={this.state.showForm}>
              <form>
                ...
                <textarea
                  onChange={f => {
                    this.setState({ r2: f.target.value })
                  }}
                  value={r2}
                  id="response2"
                />
                <button id="submitBtn" onClick={this.submit}>
                  Submit
                </button>
              </form>
            </Fade>
